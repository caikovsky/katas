...
class StringCalculatorKtTest {
    ...
    @Nested
    @DisplayName("three or more digits")
    inner class ThreeOrMoreDigits {
        @Test
    ...
    inner class CustomDelimiter {
        ...
        @Test
        fun `returns error when the delimiter used is a single character and different from the one that was set`() {
            assertEquals("'|' expected but ',' found at position 3.", add("//|\n1|2,3"))
        }

        @Test
        fun `returns error when the delimiter used is a string and different from the one that was set`() {
            assertEquals("'|' expected but 'sep' found at position 3.", add("//|\n1|2sep3"))
        }

        @Test
        fun `returns error when the delimiter used is a string, has uppercase letters and different from the one that was set`() {
            assertEquals("'|' expected but 'sEp' found at position 3.", add("//|\n1|2sEp3"))
        }
    }
